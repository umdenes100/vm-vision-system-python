...
kill_port() {
    ...
    if command -v fuser >/dev/null 2>&1; then
        ...
        pids="$(
            fuser -n "$proto" "$port" 2>/dev/null || true
        )"
        ...
        echo "[run] WARNING: neither fuser nor lsof is installed"
        return 0
    fi
    ...
    for _ in {1..20}; do
        ...
        if [[ "$alive" -eq 0 ]]; then
            echo "[run] Port ${port} process terminated"
            return 0
        fi
    ...
    echo "[run] PID(s) still alive; sending SIGKILL"
...
shutdown_wrapper() {
    ...
    echo
    echo "[run] Supervisor shutdown requested"


    if [[ -n "$CURRENT_PID" ]] && \
       kill -0 "$CURRENT_PID" 2>/dev/null; then

        echo "[run] Sending SIGINT to PID $CURRENT_PID"
...
echo "============================================================"
echo "[run] Vision system supervisor started"
echo "============================================================"
...
while true; do
    ...
    # Give Linux a moment to completely release sockets.
    sleep 0.25
    ...
    # Always sweep the application ports after Python exits.
    #
    # This catches:
    #   - orphaned GStreamer
    #   - orphaned WebSocket server
    #   - anything else still holding one of our ports
    ...
    if [[ "$STATUS" -eq "$RESTART_EXIT_CODE" ]]; then
        ...
        echo "[run] Waiting for sockets/processes to settle..."
